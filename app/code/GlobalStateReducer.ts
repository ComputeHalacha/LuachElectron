...
export default function globalStateReducer(
  ...
  switch (action.type) {
    ...
    case 'UPDATE_ENTRY': {
      const updatedList = [...state.appData.EntryList.list];
    ...
    case 'ADD_ENTRY': {
      return {
        ...
        appData: {
          ...state.appData,
          EntryList: [...state.appData.EntryList.list, action.payload]
        }
      };
    }
    case 'REMOVE_ENTRY': {
      const filteredList = state.appData.EntryList.list.filter(
